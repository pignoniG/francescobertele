...
<!--
		<a class="facebook" href="https://www.facebook.com/sharer.php?u=" target="_blank" rel="nofollow"><i class="fab fa-facebook-f"></i></a>

		<a class="twitter" href="https://twitter.com/intent/tweet?url=" target="_blank" rel="nofollow"><i class="fab fa-twitter"></i></a>
		-->
    ...
    <p class="_BodyText" style="border-bottom: 1px dashed #FFF;"><br><br><?php echo  wpm_translate_string( "[:en]DATE[:it]DATA[:]", $language = '' ); ?>:<br><?php the_field('project_date'); ?><br></p>

	<?php endif;?>

	<?php if( get_field('project_corpus') ):?>
    <p class="_BodyText" style="border-bottom: 1px dashed #FFF;">CORPUS:<br><?php the_field('project_corpus'); ?><br> <a href="<?php echo wpm_translate_url( get_page_link(74), $language = '' );?>/?force=True#filter=.project_corpus_<?php echo myUrlEncode (filter_var(get_field("project_corpus")[0], FILTER_SANITIZE_URL))  ?>
    ...
    <p class="_BodyText" ><?php echo  wpm_translate_string( "[:en]SOURCES AND DOCUMENTS[:it]FONTI E DOCUMENTI[:]", $language = '' ); ?>:
    	   ...
    	   <?php if( have_rows('project_other_links') ):
		    ...
		    while( have_rows('project_other_links') ) : the_row();
		    	...
		    	$project_other_link = get_sub_field('project_other_link');
